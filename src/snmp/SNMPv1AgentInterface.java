...
// note: dis class got a bit problem, i shall modify it..no nit to create so many thread to handle!

//add listener vector also useless!
package snmp;
...
public class SNMPv1AgentInterface
                    implements Runnable
{
    public static final int SNMP_PORT = 90;
        ...
        while (!receiveThread.isInterrupted())
        {
            ...
            try
            {
                ...
                */
                ...
                SNMPSequence receivedVarBindList = receivedPDU.getVarBindList();

                Hashtable variablePairHashtable = new Hashtable();
                SNMPSequence responseVarBindList = new SNMPSequence();
                int errorIndex = 0;
                ...
                try
                {
                    ...
                    if ((requestPDUType == SNMPBERCodec.SNMPGETREQUEST) || (requestPDUType == SNMPBERCodec.SNMPSETREQUEST))
                    {
                        ...
                        for (int i = 0; i < listenerVector.size(); i++)
                        {
                            ...
                            SNMPSequence handledVarBindList = listener.processRequest(receivedPDU, communityName, inPacket);

                            
                            //roy: usually equals to 1...a loop is for expanding purpose.
                            for (int j = 0; j < handledVarBindList.size(); j++)
                            {

                                SNMPSequence handledPair = (SNMPSequence)handledVarBindList.getSNMPObjectAt(j);
                        ...
                        //roy , after process. put inside hashtable as record...check with original...if not exist.= not handle..failed!
                        for (int j = 0; j < receivedVarBindList.size(); j++)
                        {
                            SNMPSequence requestPair = (SNMPSequence)receivedVarBindList.getSNMPObjectAt(j);
                            ...
                            if (!variablePairHashtable.containsKey(snmpOID))
                            {
                                ...
                                //System.out.println("VALUE_NOT_AVAILABLE" + errorIndex + " " + errorStatus);
                            ...
                            responseVarBindList.addSNMPObject(responsePair);
                    ...
                    else if (requestPDUType == SNMPBERCodec.SNMPGETNEXTREQUEST)
                    {
                        ...
                        for (int i = 0; i < listenerVector.size(); i++)
                        {
                            ...
                            SNMPSequence handledVarList = listener.processGetNextRequest(receivedPDU, communityName, inPacket);
                ...
                catch (SNMPRequestException e)
                {
                    ...
                    //System.out.println("SNMPRequestException" + errorIndex + " " + errorStatus);
                    // just return request variable list as response variable list
                    //e.printStackTrace();
                   // System.out.println("enter SNMPRequestException e");
                    responseVarBindList = receivedVarBindList;
                }
                catch (Exception e)
                {
...
// System.out.println("enter Exception e");
                ...
                SNMPPDU pdu = new SNMPPDU(SNMPBERCodec.SNMPGETRESPONSE, requestID, errorStatus, errorIndex, responseVarBindList);
            ...
            catch (IOException e)
            {
               ...
               // e.printStackTrace();
               //  System.out.println("enter IOException e");
                errorLog.flush();
            }
            catch (SNMPBadValueException e)
            {
              ...
              //  e.printStackTrace();
              //   System.out.println("enter SNMPBadValueException e");
                errorLog.flush();
            }
            catch (Exception e)
            {
             ///   e.printStackTrace();
             //   System.out.println("enter Exception e last problem");
                // just report the problem
